      ...
      <div v-if="dialogStatus">
        <el-form :model="newProd" :rules="formRules" ref="roleFrom">
          <el-form-item label="选择酒店" :label-width="labelWidth" prop="companyId">
            <el-select v-model="newProd.companyId" placeholder="请选择..." @change="selectHotel">
              <el-option
          ...
          <el-form-item label="座椅标签" :label-width="labelWidth" prop="hotelTableId">
            <el-select v-model="newProd.hotelTableId" placeholder="请选择...">
              <el-option
                v-for="(item,index) in newTableLabelList"
                :key="index"
                :label="item.name"
                :value="item.id"
              ></el-option>
...
export default {
  ...
  methods: {
    selectHotel(a) {
      console.log(123123)
      this.newProd.hotelTableId = ''
      this.newTableLabelList = this.tableLabelList.filter(
  ...
  watch: {
    newProd(newValue, oldValue) {
      if (newValue.companyId) {
        this.newTableLabelList = this.tableLabelList.filter(
          item => item.companyId === newValue.companyId
        )
      }
    }
  }
