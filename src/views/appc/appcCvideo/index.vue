<template>
  <div id="appcCvideo" class="mainWrap">
...
<script>
import { getVideoInfoList, deleteVideoInfo } from '@/api/member'

import addCVIdeo from './addCVIdeo'
...
export default {
  ...
  methods: {
    ...
    getList: async function() {
      ...
      let res = await getVideoInfoList(obj)
      if (res.code == 200) {
        let { result } = res
      ...
      switch (type) {
        ...
        case '删除':
          this.$confirm('确认删除？', '提示', {
            ...
            .then(() => {
              let videoIdList = val.videoId

              deleteRoleInfo({ videoIdList }).then(res => {
                if (res.code == 200) {
                  this.$message({
                    type: 'success',
                    message: '删除成功'
        ...
        case '批量删除':
          ...
          if (selectData.length === 0) {
            ...
            this.$confirm('确认批量删除选中的？', '提示', {
              ...
              .then(() => {
                let videoIdList = selectData.map(i => i.videoId)
                // videoIdList = videoIdList.toString()
                deleteVideoInfo({ videoIdList }).then(res => {
                  if (res.code == 200) {
                    this.$message({
                      type: 'success',
                      message: '删除成功'
